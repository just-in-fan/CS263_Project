# pythondj_make_plots.py
import pandas as pd
import matplotlib.pyplot as plt

# 1) Load the CSV
df = pd.read_csv("python_dijkstra_bench.csv")

# Clean up algorithm names (remove '_py' suffix)
df["algorithm"] = df["algorithm"].str.replace('_py', '', regex=False)
...
for prob in edge_probs:
    ...
    plt.title(f"Python Dijkstra Median Runtime vs Number of Nodes (edge_prob={prob})", fontsize=14, pad=15)
    ...
    plt.savefig(f"python_dijkstra_runtime_n_vs_p{prob_str}.png", dpi=300, bbox_inches="tight")
    plt.close()

print("Individual plots saved as 'python_dijkstra_runtime_n_vs_p<prob>.png'")
...
plt.suptitle("Python Dijkstra Algorithm Performance Across Different Edge Probabilities", 
             fontsize=16, y=1.02)
plt.tight_layout()
plt.savefig("python_dijkstra_runtime_all_probs.png", dpi=300, bbox_inches="tight")
plt.close()

print("Comparison plot saved as 'python_dijkstra_runtime_all_probs.png'")
